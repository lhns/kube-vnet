...
> **Reference status.** This is the original design document. Where it disagrees with the current implementation (notably: baseline is uniformly deny-all selecting every pod; pod join labels are inputs to a resolution layer that stamps `kube-vnet.system/net.<vnet>` labels read by the generator; the join label value carries a *direction*; long-form labels are accepted in the home namespace; `VirtualNetworkBinding` and the new `ClusterVirtualNetworkBaseline` are the no-label alternative; the `kube-vnet/ingress-isolation` annotation and `--ingress-isolation*` flags have been removed; the `--elide-baseline-for` flag from ADR 0030 was removed by [ADR 0035](../adr/0035-removal-of-elide-baseline-for.md); operator-owned labels live under the `kube-vnet.system/` prefix per ADR 0037; policies use kind-prefixed names — `kube-vnet.base`, `kube-vnet.mem.*`, `kube-vnet.ext.*` — per ADR 0039; the auto-allow family for externally-exposed Services, hostPort pods, and apiserver-dialed webhook/APIService backends was added by ADRs 0038/0040/0041; and the optional admission webhook and network wait were added by ADRs 0034/0045), the [ADRs](../adr/README.md) are authoritative. See in particular ADRs 0021 onward.
