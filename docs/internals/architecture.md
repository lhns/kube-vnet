...
- **`ResolutionReconciler`** — watches `Pod`, `Namespace`, `ClusterVirtualNetworkBaseline`, `VirtualNetworkBaseline`, and `VirtualNetworkBinding`. Computes effective `(vnet, direction)` per pod via the three-tier inheritance lattice (cluster baseline → namespace baseline → pod tier; intersection on within-tier conflicts) and patches `kube-vnet.system/net.<vnet>=<direction>` labels onto pods. ADRs 0030 + 0031.
- **`JoinLabelDiagnosticReconciler`** — watches `Pod`; emits the pod-scoped Warning events (`BareJoinLabelVnetNotFound`, `PrefixedJoinLabelVnetNotFound`, `JoinLabelNamespaceNotAllowed`) so label mistakes surface in `kubectl describe pod`. ADR 0027.
