...
- **`NamespaceReconciler`** — watches `Namespace`. **Sole owner** of the baseline lifecycle. Resolves each namespace's `ingress-isolation` mode (annotation → operator-level override list → cluster-wide default) and applies / removes the baseline accordingly.
...
| `NamespaceReconciler` | The baseline (`kube-vnet-default-deny`) lifecycle in every managed namespace | `Namespace` events |

This split is deliberate. The `VirtualNetworkReconciler` is purely about membership; it never inspects or writes the baseline. The `NamespaceReconciler` is purely about the baseline; it never inspects vnets. Baseline existence is decided purely by the resolved `ingress-isolation` mode for the namespace — there is no implicit "first vnet member triggers the baseline" coupling. See [ADR 0023](adr/0023-decoupled-disabled-and-ingress-isolation.md).
...
The `kube-vnet-default-deny` baseline is owned exclusively by the `NamespaceReconciler`. Its presence and shape are decided by the namespace's resolved `ingress-isolation` mode:

- Mode `none` → no baseline.
- Mode `namespace` → baseline with `policyTypes: [Ingress]`, allowing ingress from same-namespace pods.
- Mode `pod` → baseline with `policyTypes: [Ingress]`, no ingress allow rules (strict deny).

Egress is unrestricted by the baseline. Membership policies still grant egress allows to vnet peers; everything else (DNS, the apiserver, the public internet, other namespaces) is not blocked by kube-vnet. See [ADR 0025](adr/0025-ingress-isolation-rename-egress-unrestricted.md).

Resolution order, highest precedence first:

1. `kube-vnet/ingress-isolation` annotation on the namespace.
2. Operator-level override lists (`--ingress-isolation-{none,namespace,pod}`).
3. Cluster-wide default (`--ingress-isolation`, default `none`).

`kube-vnet/disabled=true` and `--excluded-namespaces` membership override everything: nothing kube-vnet-related happens in those namespaces.
