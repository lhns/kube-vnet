...
- **`NamespaceReconciler`** — watches `Namespace`. **Sole owner** of the baseline lifecycle. Applies the deny-all baseline (with `--elide-baseline-for` exemptions) to every managed namespace; bypasses disabled namespaces entirely. See [ADR 0030](adr/0030-unified-vnet-membership-with-resolution.md).
...
- **`ResolutionReconciler`** — watches `Pod`, `ClusterVirtualNetworkBinding`, and `VirtualNetworkBinding`. Computes effective `(vnet, direction)` per pod via the inheritance lattice and patches `kube-vnet.system/net.<vnet>=<direction>` labels onto pods. ADR 0030.
...
- **`baseline.go:DesiredBaseline`** — returns the deny-all baseline policy spec for a given namespace, with `podSelector` matchExpressions excluding pods that are receivers on any vnet listed in `--elide-baseline-for`. Pure.
...
| `ResolutionReconciler` | The `kube-vnet.system/net.<vnet>=<direction>` labels stamped on every managed pod | `Pod`, `ClusterVirtualNetworkBinding`, `VirtualNetworkBinding` events |

This split is deliberate. The `VirtualNetworkReconciler` is purely about membership; it never inspects or writes the baseline. The `NamespaceReconciler` is purely about the baseline; it never inspects vnets. The baseline shape is uniformly deny-all minus `--elide-baseline-for` exemptions; per-NS variation comes from the *pods* in the namespace (which vnets they belong to), not from per-NS config. See [ADR 0023](adr/0023-decoupled-disabled-and-ingress-isolation.md) and [ADR 0030](adr/0030-unified-vnet-membership-with-resolution.md).
...
2. Build one or more policies per (namespace, direction class, label form): name `kube-vnet-<vnet>-<ns>` for the bidirectional, bare-form policy, with `-ingress` / `-egress` direction suffixes and `-prefixed` form suffixes as needed (truncate-and-hash if > 253 chars per [ADR 0011](adr/0011-policy-naming-and-truncation.md)); labels `kube-vnet/managed-by=kube-vnet, kube-vnet/network=<homeNS>.<vnet>, kube-vnet/role=membership` (plus `kube-vnet/binding=<binding>` on per-binding policies); the namespace's join-key form as the `podSelector` with an `In` value match for the direction class; peer rules in `ingress[0].from` and/or `egress[0].to`. There is no DNS allow rule — egress is unrestricted by the baseline so DNS works without one (see [ADR 0025](adr/0025-ingress-isolation-rename-egress-unrestricted.md)).
...
- `podSelector` excludes pods that are receivers (`In: [both, ingress]`) on any vnet listed in `--elide-baseline-for` (default: `cluster`). Excluded pods don't get the deny-all; their effective ingress is whatever their membership policies grant.
