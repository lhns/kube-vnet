...
- **`ResolutionReconciler`** — watches `Pod`, `ClusterVirtualNetworkBaseline`, `VirtualNetworkBaseline`, and `VirtualNetworkBinding`. Computes effective `(vnet, direction)` per pod via the three-tier inheritance lattice (cluster baseline → namespace baseline → pod tier; intersection on within-tier conflicts) and patches `kube-vnet.system/net.<vnet>=<direction>` labels onto pods. ADRs 0030 + 0031.
