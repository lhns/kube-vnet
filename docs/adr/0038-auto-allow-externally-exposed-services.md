...
> **Amendment (2026-06-29)**: the emitted policy's `from:` rule now includes BOTH `ipBlock: 0.0.0.0/0` AND `namespaceSelector: {}` (empty selector → all namespaces). Many CNIs (Calico, Cilium, kube-router in some configs) treat `ipBlock` peers as off-cluster-source-only and do NOT match cluster-internal IPs (node IPs, pod IPs) against them — even `ipBlock: 0.0.0.0/0`. Cluster-internal sources that legitimately reach LB/NodePort backends (kube-proxy SNAT to the node IP for `externalTrafficPolicy: Cluster` Services, kubelet probes, in-cluster pods using the LB hostname) were being dropped. The `namespaceSelector: {}` makes the policy CNI-portable. See ADR 0041 amendment for the matching change and full reasoning.
>
