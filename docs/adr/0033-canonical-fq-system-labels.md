...
- [ADR 0043](0043-virtualnetworkref-namespace-inferred-or-honored.md): a `virtualNetworkRef` is permission-checked on its fully-qualified key first (a wrong `<ns>.cluster` is denied as not-found) and collapsed to bare `cluster` only afterwards.
- 2026-09-25: prefixed cluster **join labels** no longer collapse. `kube-vnet/net.<anything>.cluster` is invalid, `<operatorNS>.cluster` included: the pod does not join through it and gets a `VirtualNetworkNotJoinable` Warning, and the join-label ValidatingAdmissionPolicy rejects the key on CREATE and when an UPDATE adds it. Only bare `kube-vnet/net.cluster` joins. Reason: the singleton has no namespace, so a namespace in the label means nothing, and accepting any value there taught users a form that suggests a per-namespace cluster vnet. Stamps stay `kube-vnet.system/net.cluster`. The collapse below still applies to stamped identity and to `virtualNetworkRef`s, which name the vnet object in its real home namespace.
