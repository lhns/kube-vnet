...
> **Amendment (2026-09-23/24)**: since [ADR 0023](0023-decoupled-disabled-and-ingress-isolation.md) the baseline, named `kube-vnet.base` since [ADR 0039](0039-uniform-kind-prefixed-policy-naming.md), is owned by `NamespaceReconciler`; its `NetworkPolicy` watch still restores drift. Every restore (baseline, membership, auto-allow) emits `PolicyRestored` on the restored policy itself, so it lands in the namespace of whoever deleted it; a membership restore is also reported on the vnet. A membership restore is one the vnet's `status.generatedPolicies` listed; the other reconcilers remember in memory which policies they applied, so a deletion during an operator restart is restored without an Event.
