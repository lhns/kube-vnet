...
Status: Accepted. Implemented as an opt-in (`webhook.enabled`, 2026-09-11); the "VAP exemption" section below is wrong, see the amendment.
...
> **Amendment (2026-09-11, revised 2026-09-23) — implementation.**
>
> *The window, measured.* A `vmctl` migration Job failed its first connection and succeeded on retry. On kube-router v2.10.0 a kube-vnet denial is an immediate RST, and probing bounded kube-vnet's share of the window at <=1s. That is not the whole window: with the webhook on, pods were stamped at creation yet only 3 of 10 first connections succeeded, because kube-router rewrites its full ruleset on every pod event (0.77–1.85s per rewrite) and traffic worked only 2.15–3.64s after container start. The webhook removes kube-vnet's delay, not the CNI's; [ADR 0045](0045-network-wait-for-opted-in-pods.md) addresses the CNI's share for pods that opt in. A separate report, a pod created before its vnet and never stamped, was the missing `VirtualNetwork` watch fixed in [ADR 0030](0030-unified-vnet-membership-with-resolution.md)'s 2026-07-26 amendment, not this window.
>
> *Label protection.* A mutating webhook's patch is attributed to the requester, and mutating webhooks run before validating admission policies, so the `system-labels-vap` pods rule (no system labels on CREATE unless the operator's ServiceAccount writes them) would deny every stamped pod. With the webhook enabled, the pods it admits are policed by a validating webhook backed by the same resolver. It enforces more than CEL can: every `kube-vnet.system/*` label the request adds, changes or removes must match what `Resolve()` produces, and untouched stamps are not checked, so a stamp awaiting reconcile can't deny an unrelated edit. The VAP keeps its pods rule, narrowed by a `matchCondition` to what the webhooks skip: the excluded namespaces, pods labelled with the chart's `app.kubernetes.io/name` (the operator's own), and `pods/status` everywhere. *(Until 2026-09-23 the pods rule was dropped outright, which left those pods unprotected.)* The VAP's `networkpolicies` and `virtualnetworks` rules are unchanged, and with `webhook.enabled=false` the chart renders as before.
>
> *Stamp ownership.* The mutator writes and prunes only stamps the request left untouched. A stamp the request supplied stays as submitted, so the validator rejects a wrong value with guidance instead of the mutator silently rewriting it (it did until 2026-09-23). The reconciler owns every stamp. Both write through `controller.SyncStamps` and resolve through the shared `controller.Resolver`.
>
> *failurePolicy is split.* Mutating is `Ignore`: an outage degrades to pre-webhook behaviour. Validating is `Fail`, so stamps can't be forged while the operator is down. That is an availability regression: an operator outage blocks pod creation in managed namespaces, which is why the feature is opt-in. `kube-system`, `kube-public`, `kube-node-lease`, the release namespace and the `--disabled-namespaces` list are excluded so the cluster and the operator can recover. No bootstrap loop: `cmd/main.go` always adds the operator's namespace to the disabled list, so it gets no baseline and the webhook Service needs no [ADR 0041](0041-auto-allow-apiserver-reachable-services.md) auto-allow.
>
> *Scheduling gates were rejected (2026-09-23).* Gating pods at admission and letting the reconciler stamp and ungate them would keep the admission policy as the only protection model and drop the dependency on a `failurePolicy: Fail` webhook, but it delays every pod start by a reconcile and can't stamp a relabelled running pod synchronously. Both webhook modes run the full e2e suite.
>
> *Corrections to the text below.* [ADR 0035](0035-removal-of-elide-baseline-for.md) removed the baseline's `NotIn` matchExpression: the baseline selects every pod and membership policies grant, so the fail-closed property is unchanged. The "Handler shape" shipped as `mutator.go` and `validator.go` in `internal/webhook/podresolution/`. The reconciler rewrites the marker annotations only when it has to patch the pod, not on every re-resolve. Restamps caused by Binding, Baseline or VirtualNetwork edits stay asynchronous. `BareJoinLabelVnetNotFound` and `PrefixedJoinLabelVnetNotFound` (under "Out of scope") were pod Events, not `Degraded` reasons, and are now the `VirtualNetworkNotJoinable` Warning ([ADR 0027](0027-pod-scoped-join-label-events.md)).
