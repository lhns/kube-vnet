...
> **Amendment (2026-09-23)**: in the follow-up below, `resolveTargetPort` is now `resolveTargetPorts`: a named `targetPort` resolves on every backing pod, and all the port numbers in use are allowed. The Pod watch fires on creates, deletes and label changes, not only creates ([ADR 0044](0044-trigger-sets-must-cover-read-sets.md)); later the same day narrowed to pods entering or leaving a named-port Service's selector (ADR 0044's second amendment).
