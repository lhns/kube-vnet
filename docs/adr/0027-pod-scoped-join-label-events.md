...
>
> The **admission-time VAP** surface (below) is unaffected: syntactic direction-value validation still rejects bad labels at `kubectl apply`. A reconcile-time mirror was added alongside the retirement: the resolution controller also emits an `InvalidJoinLabelDirection` Warning for any `kube-vnet/net.*` value outside the direction enum, so the mistake surfaces even on clusters without the VAP (Kubernetes < 1.30, or disabled). This also means the *removed-alias* empty string `""` now draws that Warning at reconcile time — consistent with the pruned VAP allow-list; the supported opt-out value is `none`.
>
> **~~Known gap, deliberately accepted for now~~ — CLOSED (2026-07-26)** by the `VirtualNetwork` watch added to the `ResolutionReconciler`; see [ADR 0030](0030-unified-vnet-membership-with-resolution.md)'s 2026-07-26 amendment. A field incident (a pod created before its vnet, reported as needing a pod roll to recover) supplied the "measured need" this paragraph deferred on. **Two corrections to what follows:** it said the pod would be stuck "until some other event touches the pod (**or informer resync**)" — the resync half is **wrong**. The pod predicate is change-based and the resync delivers `old == new`, so those events are filtered; the pod was stuck *indefinitely*. And the impact was understated: it is not only a stale warning, the pod never gets its membership **stamp**, so it stays isolated by the deny-all baseline. Original text follows.
>
> **The `ResolutionReconciler` does not watch `VirtualNetwork`.** Its triggers are Pod (label/annotation/generation changes), the two baselines, `VirtualNetworkBinding`, and Namespace annotations. Consequence: *create the pod first, the vnet second*, and nothing re-resolves the pod — its `VirtualNetworkNotJoinable` Warning stays stale **and its membership stamp stays missing** until some other event touches the pod (or informer resync). This is not a regression — v0.6.0 behaved identically for the stamp, and the retired controller's `Watches(&VirtualNetwork{})` (2026-07-09 amendment below) only ever re-checked the *warning*, never the stamp. The clean fix is that same watch moved onto **resolution**: map a VirtualNetwork event to pods via the two statically-known label keys (bare `kube-vnet/net.<name>` scoped to the vnet's namespace; prefixed `kube-vnet/net.<ns>.<name>` cluster-wide) — two label-selected Lists, no cluster scan — which would repair both the stale warning and the missing-stamp delay in one step. Deferred until measured need; recorded here so the shape isn't lost.
