...
> **Amendment (2026-09-23) — restating a pinned value is not an override.** Only a lower tier that names a bare-pinned vnet with a *different* direction is rejected. One that repeats the pinned value (a namespace baseline or pod label saying `none` for the `payments` example below) is a no-op and emits no `OverrideRejected` Event, so "at any direction" in that example should read "at any other direction".
