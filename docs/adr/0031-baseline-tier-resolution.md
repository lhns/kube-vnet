...
> **Amendment (2026-09-23) — conflicts and rejected overrides are Warning Events on the pod.** The surfaces described below (a `kube-vnet.system/conflict.<vnet>` pod annotation, a `kube_vnet_resolution_conflicts_total` metric, `Conflicts`/`OverrideRejected` baseline conditions) were never built and are dropped. The resolution reconciler, which runs for every pod including webhook-stamped ones, emits `ResolutionConflict` (same-tier rules intersected) and `OverrideRejected` (a lower tier tried to change a bare-pinned value) on the pod, naming the vnet, the sources and the result, like `VirtualNetworkNotJoinable` and `InvalidDirection`; baselines need no status writer. A lower tier that restates the pinned value is a no-op and emits nothing, so "at any direction" in the `payments` example below should read "at any other direction". The resolution rules are unchanged. *(Correction: the code has three scopes, not the four listed under "Decision": bindings and pod labels share `ScopePod`, matching the sibling-intersection rule below.)*
