...
**System label** — an operator-stamped label of the form `kube-vnet.system/net.<vnet>=<direction>`. Written by the `ResolutionReconciler` based on the inheritance lattice (operator defaults → ClusterVirtualNetworkBinding → VirtualNetworkBinding → user-authored join label). Read by the policy generator's selectors. Protected from user mutation by a ValidatingAdmissionPolicy. See [ADR 0030](adr/0030-unified-vnet-membership-with-resolution.md).
...
**`ClusterVirtualNetworkBinding`** — a cluster-scoped CRD (short names `cvnb`, `cvnbs`) introduced in [ADR 0030](adr/0030-unified-vnet-membership-with-resolution.md). Selects pods via `namespaceSelector` + `podSelector`; applies the chosen `direction` to the matched pods as an inheritance-layer input the resolution controller stamps onto pods.

**`ResolutionReconciler`** — the controller-runtime reconciler in `internal/controller/resolution_controller.go` that watches `Pod`, `ClusterVirtualNetworkBinding`, and `VirtualNetworkBinding`. Computes the effective `(vnet, direction)` per pod via the inheritance lattice and patches `kube-vnet.system/net.<vnet>=<direction>` labels onto pods. See [ADR 0030](adr/0030-unified-vnet-membership-with-resolution.md).
