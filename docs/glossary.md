...
**Disabled namespace** — a namespace the operator explicitly does nothing in. Disabled via the `--disabled-namespaces` flag / `operator.disabledNamespaces` Helm value (default `[]`, plus the operator's own namespace added implicitly) or per-namespace via the `kube-vnet/disabled=true` annotation. The control-plane namespaces (`kube-system`, `kube-public`, `kube-node-lease`) used to be disabled by default; they are now in `operator.ingressIsolation.namespaceOverrides.none` instead — discovered but never baseline'd. See ADRs [0006](adr/0006-baseline-default-deny-and-single-opt-out.md) and [0007](adr/0007-operator-level-excluded-namespaces.md).
