...
- **Vnet condition**: the vnet's `Degraded=True, reason=InvalidJoiners` names offending pods with a per-pod reason (`UnknownDirection`, `NamespaceExcluded`, `NamespaceNotAllowed`).
