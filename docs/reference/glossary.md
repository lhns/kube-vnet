...
**Direction** — the value carried by a join label, declaring which directions the labeled pod participates in. At the pod tier (labels, bindings): exactly one of `both` (default), `ingress`, `egress`, `none`. Baseline tiers additionally accept `default-both`/`default-ingress`/`default-egress`/`default-none` — advisory values lower tiers may override (bare values are enforced; see [ADR 0031](../adr/0031-baseline-tier-resolution.md)). The legacy `"true"`/`"false"`/empty-string aliases were dropped per [ADR 0030](../adr/0030-unified-vnet-membership-with-resolution.md) (see also the [ADR 0021 2026-05-05 addendum](../adr/0021-direction-modes-on-join-labels.md#addendum-2026-05-05--legacy-truefalseempty-aliases-dropped)). Unknown values surface as `Degraded`/`UnknownDirection`. Traffic algebra: X→Y iff X is initiator-capable (`both`/`egress`) AND Y is receiver-capable (`both`/`ingress`).
...
**Prefixed label form** — a join label with the home namespace baked into the key: `kube-vnet/net.<homeNS>.<vnet-name>=<direction>` (e.g. `=both`). Used by pods *in any namespace other than the VirtualNetwork's home namespace*. Compare *bare label form*.
