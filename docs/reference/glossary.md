...
**InvalidJoiner** — a pod in a namespace a vnet admits whose join label for it can't be honored: unknown direction value or disabled namespace. Surfaced on the VirtualNetwork's `Degraded` condition with reason `InvalidJoiners` and a per-pod sub-reason (`UnknownDirection`, `NamespaceExcluded`). A pod in a namespace the vnet doesn't admit is not an InvalidJoiner; it gets a `VirtualNetworkNotJoinable` Event.
