...
**Direction** — the value carried by a join label, declaring which directions the labeled pod participates in. At the pod tier (labels, bindings): exactly one of `both`, `ingress`, `egress`, `none` (a binding's `direction` defaults to `both`). Baseline tiers additionally accept `default-both`/`default-ingress`/`default-egress`/`default-none` — advisory values lower tiers may override (bare values are enforced; see [ADR 0031](../adr/0031-baseline-tier-resolution.md)). The legacy `"true"`/`"false"`/empty-string aliases were dropped per [ADR 0030](../adr/0030-unified-vnet-membership-with-resolution.md) (see also the [ADR 0021 2026-05-05 addendum](../adr/0021-direction-modes-on-join-labels.md#addendum-2026-05-05--legacy-truefalseempty-aliases-dropped)). Unknown label values are rejected at admission (Kubernetes ≥ 1.30) or ignored with an `InvalidDirection` Event. Traffic algebra: X→Y iff X is initiator-capable (`both`/`egress`) AND Y is receiver-capable (`both`/`ingress`).
...
**InvalidJoiner** — a pod in a namespace a vnet admits whose join label for it can't be honored: unknown direction value or disabled namespace. Surfaced on the VirtualNetwork's `Degraded` condition with reason `InvalidJoiners` and a per-pod sub-reason (`InvalidDirection`, `NamespaceExcluded`). A pod in a namespace the vnet doesn't admit is not an InvalidJoiner; it gets a `VirtualNetworkNotJoinable` Event.
