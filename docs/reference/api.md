# API reference: VirtualNetwork

Full reference for the `VirtualNetwork` CRD. For the conceptual model see [`../concepts.md`](../concepts.md); this document is for look-up.
...
| False | `HomeNamespaceExcluded` | "home namespace <ns> is excluded by the operator" | The vnet's home namespace is in `--excluded-namespaces` or has `kube-vnet/disabled=true`. |
