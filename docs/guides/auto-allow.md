...
Not an operator family: CoreDNS's `:53` is a plain ClusterIP Service, matched by none of the families above. When you enroll `kube-system`, the **chart** renders `kube-vnet-coredns-allow`, a raw `ipBlock: 0.0.0.0/0` policy on `:53`. See [recipes § managing kube-system](recipes.md#managing-kube-system-and-keeping-dns-alive) and [ADR 0042](../adr/0042-coredns-ingress-carveout-and-kube-system-enrollment.md).
