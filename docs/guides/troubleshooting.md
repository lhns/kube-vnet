...
**Symptom.** The pod has `kube-vnet/net.<ns>.cluster` and isn't a `cluster` member. Its Event reads `pod label kube-vnet/net.<ns>.cluster: the cluster network has no namespace; use kube-vnet/net.cluster`. On Kubernetes ≥ 1.30 such a label is rejected at admission when a pod is created with it or an update adds it.
