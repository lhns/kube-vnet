...
| Read foreign-namespace pod names via `status.members` / `Degraded` messages on a vnet | A1 with `view` on the vnet | None | Members opted in by labelling, but pod names of other namespaces do leak to the vnet owner | **F-06** (Low, Open) |
...
| **F-06** | `status.members` and `Degraded` messages enumerate foreign-namespace pod names to anyone with `view` on the vnet. | I | Low | **Open.** Low priority. Mitigation if it matters: restrict `view` on vnets, or truncate pod names in status. |
