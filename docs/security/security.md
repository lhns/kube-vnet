...
Every release ships SPDX 2.3 JSON SBOMs for both the image and the chart. They're attached as Cosign attestations *and* as plain GitHub release assets. See [`install.md`](../getting-started/install.md#verifying-sboms).

- **`kube-vnet-image.sbom.spdx.json`** is generated by syft from the pushed image and lists the Go modules and OS files inside it.
- **`kube-vnet-chart.sbom.spdx.json`** describes the Helm chart and what it deploys. syft sees nothing inside a chart, so [`scripts/chart-sbom.py`](../../scripts/chart-sbom.py) builds it by rendering the chart twice, once with default values and once with every optional feature enabled. It contains:
  - the chart package: name, version, SHA-256 of the `.tgz`, and its OCI location (`oci://ghcr.io/lhns/charts/kube-vnet`) as a `pkg:oci` purl with the pushed digest;
  - one package per container image the chart can deploy, with its tag and manifest digest. Today these are the operator image `ghcr.io/lhns/kube-vnet`, which also runs the network-wait beacons and init containers, and `registry.k8s.io/kubectl` for the uninstall cleanup hook (`cleanup.image.*`). If a digest can't be looked up at build time, the package's comment says so;
  - a `DEPENDS_ON` relationship from the chart to each image deployed with default values. An image that only an optional feature deploys is an `OPTIONAL_DEPENDENCY_OF` the chart instead;
  - an `externalDocumentRefs` entry that points at the image SBOM by its document namespace and SHA-1. The operator image package is `DESCRIBED_BY` that document, so a consumer can follow the chart to the image's contents.
